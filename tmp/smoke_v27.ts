...
ok(BALANCE_VERSION === "v27", "version", BALANCE_VERSION);
for (const id of ["NWL4", "NSPR", "CREATOR", "GM7_0", "GM10_0", "CULL_TITAN", "WORLD_GUARD"]) ok(!DB[id], `deleted ${id}`);
ok(!BUYABLE_POOL.includes("GM9_2") && !!DB.GM9_2, "GM9_2 token only");
...
ok(!!DB.NGA4.textEn && DB.NGA4.textEn.includes("Dual Wield"), "en text", DB.NGA4.textEn);
...
// 침묵 4
{ let g = fresh(); g.players[1].field.push(mk("NT_SEAL3")); const p = g.players[0]; p.hand.push(card("S11")); p.maxMana = 10; p.mana = 10; const n = p.hand.length; g = reduce(g, { type: "play", idx: n - 1 }).state; ok(g.players[0].hand.length === n, "seal4 blocks cost2");
...
// 장군
{ let g = fresh(); g = play(g, "GM6_7"); ok(g.players[0].field.some((m) => m.id === "INFKNIGHT"), "general knight"); g = reduce(g, { type: "endTurn" }).state; let got = false; for (let i = 0; i < 6 && !got; i++) { g = play(g, "M1"); got = g.players[0].field.filter((m) => m.id === "INFKNIGHT").length >= 2; } ok(got, "general dice knight"); }
// 공허의 공성병: 사망 시 병사
...
for (let seed = 1; seed <= 80; seed++) {
  ...
  try {
    while (!g.over && steps < 4000) {
      const a = seed % 3 === 0 ? botDecide(g) : greedyDecide(g);
