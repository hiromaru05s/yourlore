...
import { createGame, reduce, effMaxMana, MIN_MANA, HAND_CARRY, MARKET_STOCK, marketStockOf } from "../client/src/shared/engine";
...
const end = (g: GameState): GameState => { const hp = g.players[g.cur].hand; if (hp.length > 5) hp.length = 5; return reduce(g, { type: "endTurn" }).state; }; // v42: trims to the carry-over cap first
...
{ let g = fresh(3); ok(g.players[0].hand.length === 3, "p0 opening hand 3", g.players[0].hand.length);
  ...
  ok(g.players[0].hand.length === h0 + 3, "p0 turn 3 draws 3 and keeps hand", [h0, g.players[0].hand.length]);
  const h1 = g.players[1].hand.length; g = end(g);
  ok(g.players[1].hand.length === h1 + 3, "p1 turn 4 draws 3 and keeps hand", [h1, g.players[1].hand.length]);
...
// v42: no draw-time cap — a big hand stays in hand until the end-turn carry-over choice
{ let g = fresh(5); const p = g.players[0]; while (p.hand.length < 8) p.hand.push(card("GRAPE")); p.deck.push(card("M1"));
  g = reduce(g, { type: "endTurn" }).state; // hand 8 > HAND_CARRY → discard choice pending (raw endTurn, no trim)
  ok(g.pending?.reason === "handCap" && g.cur === 0, "end turn with 8 cards asks which to discard", g.pending?.reason);
  g = reduce(g, { type: "endTurn" }).state; // timeout: forced → rightmost discarded, turn ends
  ok(g.cur === 1 && g.players[0].hand.length === HAND_CARRY, "forced end: rightmost discarded to carry-over", [g.cur, g.players[0].hand.length]); }
// Mana floor 3
