...
export async function getUser(env: Env, req: Request): Promise<SessionUser | null> {
  ...
  const row = await env.DB.prepare(
    `SELECT u.id, u.email, u.display, u.wins, u.losses, u.credits, u.avatar, u.badge, u.sleeve, s.expires_at
     FROM sessions s JOIN users u ON u.id = s.user_id WHERE s.token = ?`
  ).bind(token).first<{ id: string; email: string; display: string; wins: number; losses: number; credits: number; avatar: string | null; badge: string | null; sleeve: string | null; expires_at: number }>();
  if (!row || row.expires_at < Date.now()) return null;
  return { id: row.id, email: row.email, display: row.display, wins: row.wins, losses: row.losses, credits: row.credits, avatar: row.avatar, badge: row.badge, sleeve: row.sleeve || "default" };
}
