...
const directory='client/public/models/library-furniture';
const results=[];
for(const name of ['shelf','shelf-low','market','market-low']){
 ...
 const root=g.nodes.find(n=>n.name===(shelf?'cosmetic_root':'market_root'));assert(root);assert.deepEqual(root.translation||[0,0,0],[0,0,0]);assert.deepEqual(root.rotation||[0,0,0,1],[0,0,0,1]);assert.deepEqual(root.scale||[1,1,1],[1,1,1]);
 assert(g.nodes.some(n=>n.name===(shelf?'shelf_mount':'market_mount')));
 ...
 for(const mesh of g.meshes)for(const p of mesh.primitives){
  ...
  for(let i=0;i<a.count;i++){
   ...
   const [x,y,z]=xyz;
   if(shelf){assert(Math.abs(x)<=1.00001&&y>=-.12001&&y<=1.15001&&Math.abs(z)<=.70001,'shelf external envelope');assert(!(Math.abs(x)<.83&&y>1e-5&&y<1.12&&z>-.61&&z<.59),'protected card cavity');}
  }
 }
 assert(triangles<=(low?2000:8000));assert(draws<=(low?1:3));assert(bytes.length<512*1024);
...
await fs.writeFile('docs/3d-assets/2026-09-09-library-furniture/geometry-checks.json',JSON.stringify(results,null,2));console.log('PASS: GLB geometry, units, mounts, cavity, bounds and draw/triangle budgets',JSON.stringify(results.map(({name,bytes,triangles,draws})=>({name,bytes,triangles,draws}))));
