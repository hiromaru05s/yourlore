...
const origin=process.env.LORE_TEST_ORIGIN||'http://127.0.0.1:5182',out='docs/card-art-releases/2026-09-10-biblion';
...
try{
 ...
 for(const r of result){for(let i=0;i<3;i++){assert(r.urls[i].includes(`/${r.id}.webp?v=20260910-biblion`));assert.equal(r.sizes[i][0],[192,384,832][i]);}}
 await page.evaluate(()=>{
  ...
  const ids=['STARTER_TRASH','STARTER_CHEST','STARTER_MANA','M1','M2','M4','MIMIC','ELF','Q_RIFT','Q_BRAND','Q_WINTER','Q_MANA','QUICK_MIMIC','QUICK_ATTUNE','QUICK_GRIMOIRE','QUICK_ASSAULT','M10','CASTLE','RIFT','ACID_RAIN','GHOST','FLAME','NHEAL','S12'];
