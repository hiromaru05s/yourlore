...
const TEXTS: Record<string, string> = {
  ...
  S10: "Draw 2 cards", S11: "Deal 4 damage to the opponent", S12: "+2 ATK / +1 HP to one of your monsters (lasting)",
  ...
  GHOST: "Infiltrate · Whenever the opponent raises their max mana/your max HP, you take 2 damage",
  ...
  TRICKROOM: "For 2 turns, all monsters' ATK and HP are swapped · While swapped, HP gains raise ATK and vice versa · When it ends, stats gained while swapped are kept",
  ...
  DICE8: "Die 1-6 - 1/2: your max mana -4 / 3/4: opponent mana -1 & 14 dmg / 5: Storm Warrior (11/9, double attack) / 6: wipe enemy spells & traps + 2 Storm Warriors + max mana +2 + your HP +10",
  ...
  MEDITATE: "Castable only if you played no other card this turn. Restore your HP up to 80% of max",
  PRAYER: "Castable only if your max mana is 12 or less and you played no other card this turn. Restore your HP up to 80% of max (Cast 5)",
  ...
  TOKEN00: "Void", SOLDIER2: "Void",
  GOLEM1: "Guts", GOLEM2: "Guts",
  GOLEM3: "Guts · If you have no other 'Golem' monster in your field/deck/hand/graveyard: -4/-4 on summon",
  ...
  GAMBLER: "At the start of your turn: roll a die — on 4/5/6, max mana +1 and your max HP +5",
  ...
  D_RED: "On summon: deal 15 damage to the opponent · Your spells deal +3 extra damage to the opponent · Aura · Void",
...
const RULES: [RegExp, string][] = [
  ...
  [/^부패$/, "Decay"],
  [/^암습$/, "Infiltrate"],
  [/^매 턴 시작 시 체력 \+(\d+) 회복 · 도발$/, "At the start of each turn: restore $1 of your HP · Taunt"],
  [/^\[시초\] 동족 시너지 · 소환 코스트 (\d+)$/, "[Origin] tribe synergy. Summon cost $1"],
  [/^\[시초\] 동족 시너지$/, "[Origin] tribe synergy"],
  [/^\[시초\] 소환시 최대 체력 \+(\d+)$/, "[Origin] On summon: your max HP +$1"],
  [/^\[시초\] 소환시 최대 체력 -(\d+)$/, "[Origin] On summon: your max HP -$1"],
  [/^—$/, "—"],
  [/^공격 몬스터 파괴 \+ (\d+)%로 그 방어력만큼 회복$/, "Destroy the attacker + $1% chance to restore your HP equal to its max HP"],
  ...
  [/^공격 몬스터 파괴 \+ 체력 (\d+) 회복$/, "Destroy the attacker + restore $1 of your HP"],
  ...
  [/^공격 무효 \+ 상대 몬스터 전체 방어 -(\d+)$/, "Negate the attack + all enemy monsters -$1 HP"],
  [/^공격 무효 \+ 상대 함정·영구마법 (\d+)장 파괴$/, "Negate the attack + destroy $1 enemy traps/enchantments"],
  [/^공격 무효 \+ 자신 몬스터 전체 방어 \+(\d+) \+ (\d+)장 드로우$/, "Negate the attack + all your monsters +$1 HP + draw $2"],
  ...
  [/^매 턴 마나 (\d+)로 방어 \+(\d+), 체력 \+(\d+) 회복$/, "Each turn, pay $1 mana: this monster +$2 HP, restore $3 of your HP"],
  [/^매 턴 시작 시 공격 \+(\d+)\(지속\)$/, "At the start of each turn: +$1 ATK (lasting)"],
  [/^매 턴 시작 시 방어 \+(\d+)\(지속\)$/, "At the start of each turn: this monster +$1 HP (lasting)"],
  [/^매 턴 시작 시 상대 체력에 (\d+) 데미지$/, "At the start of each turn: $1 damage to the opponent"],
  [/^매 턴 시작 시 체력 \+(\d+) 회복$/, "At the start of each turn: restore $1 of your HP"],
  ...
  [/^상시: 자신 필드의 아군 몬스터 방어 \+(\d+)$/, "Aura: your monsters have +$1 HP"],
  ...
  [/^소환시: 적 몬스터 방어 -(\d+)\(지속\)$/, "On summon: -$1 HP to an enemy monster (lasting)"],
  [/^소환시: 체력 (\d+) 회복$/, "On summon: restore $1 of your HP"],
  [/^소환시: 최대 체력 \+(\d+), 최대 마나 \+(\d+)$/, "On summon: your max HP +$1, max mana +$2"],
  ...
  [/^체력 (\d+) 회복 · 상대 몬스터 (\d+)체 \+ 마법\/함정 (\d+)장 파괴$/, "Restore $1 of your HP. Destroy $2 enemy monster + $3 spell/trap"],
  [/^체력 (\d+) 회복 · 이 카드 (\d+)번째 사용부터 피격 시마다 체력 \+(\d+)$/, "Restore $1 of your HP. From use #$2 of this card: restore $3 of your HP whenever you're hit"],
  [/^체력 (\d+) 회복 · 자신 최대 마나가 (\d+) 이하면 체력 완전 회복$/, "Restore $1 of your HP. If your max mana is $2 or less: fully restore your HP"],
  [/^체력 (\d+) 회복 · 체력 (\d+) 이상이면 최대 체력 \+(\d+)$/, "Restore $1 of your HP. If your HP is $2+: your max HP +$3"],
  [/^체력 (\d+) 회복 · 패의 '생명의 빛' (\d+)장 묘지로 보내면 최대 체력 \+(\d+)$/, "Restore $1 of your HP. Discard $2 'Light of Life' from hand: your max HP +$3"],
  [/^체력 (\d+) 회복, (\d+)% 확률로 최대 체력 \+(\d+)$/, "Restore $1 of your HP, $2% chance of max HP +$3"],
...
Object.assign(TEXTS, {
  ...
  GUILD_CHEST: "Roll 2 dice — 2·3: max mana +3 / 4: +1 draw at turn start (permanent) / 5·6: max mana +2 / 7: max mana +1 / 8: your max HP +10 / 9·10: Novice & Adept Assassins on enemy field / 11·12: Novice, Adept & Elite Assassins + take 10 damage (Cast 3)",
