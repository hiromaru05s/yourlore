...
const PATCH13: Record<string, Partial<CardDef>> = {
  ...
  DIVINE: { passive: ["void"],
    text: "소환시: 최대 마나 +15 · 매 턴 드로우 +1(영구) · 카드 3장 선택 파괴(양측: 몬스터, 영구마법) · 아우라 · 공허",
    textJa: "召喚時: 最大マナ+15 · 毎ターンドロー+1(永続) · カード3枚を選んで破壊(両方の場: モンスター・永続魔法) · オーラ · 虚無" },
...
const PATCH29: Record<string, Partial<CardDef>> = {
  ...
  GUILD_CHEST: { text: "주사위 2개 합계 — 2·3: 최대 마나+3 / 4: 턴 시작 드로우+1(영구, 마법 존이 가득 차면 실패) / 5·6: 최대 마나+2 / 7: 최대 마나+1 / 8: 자신 최대 체력+10 / 9·10: 상대 필드에 초급·중급 암살자 소환 / 11·12: 상대 필드에 초·중·상급 암살자 소환 + 자신에게 10 데미지 (시전 3)",
    textJa: "ダイス2個の合計 — 2·3: 最大マナ+3 / 4: ターン開始ドロー+1(永続、魔法ゾーンが満杯なら失敗) / 5·6: 最大マナ+2 / 7: 最大マナ+1 / 8: 自分の最大体力+10 / 9·10: 相手の場に初級・中級アサシン召喚 / 11·12: 相手の場に初・中・上級アサシン召喚 + 自分に10ダメージ (発動3)" },
...
const PATCH30: Record<string, Partial<CardDef>> = {
  ...
  GT8_5: { cost: 6, react: "brandMagic", val: undefined,
    text: "【마법 반응】상대에게 낙인 카운터 1개 · 상대는 매 턴 시작시 낙인 카운터당 주사위 눈만큼 데미지",
    textJa: "【魔法に反応】相手に烙印カウンター1個 · 相手は毎ターン開始時、烙印カウンターごとにダイスの目のダメージ" },
...
const NEW_TRIBES32: CardDef[] = [
  ...
  { id: "TSO2", t: "mon", cost: 2, atk: 1, def: 1, tribe: "고독", onSummon: "hermitBuff", name: "외로운 늑대", nameJa: "孤独な狼",
    text: "소환시: 이 몬스터 외 자신 필드의 카드(몬스터·마법)가 1장 이하면 +3/+3",
    textJa: "召喚時: このモンスター以外の自分の場のカード(モンスター·魔法)が1枚以下なら+3/+3" },
...
const PATCH37: Record<string, Partial<CardDef>> = {
  ...
  T6: { react: "mindGame", text: "공격 무효 · 양측이 눈을 예상해 주사위 · 낙인 카운터: 상대 적중이면 자신 1개·자신 적중이면 상대 3개",
    textJa: "攻撃無効 · 互いに出目を予想しダイス · 烙印カウンター: 相手が的中なら自分に1個·自分が的中なら相手に3個" },
...
const NEW_CARDS37: CardDef[] = [
  ...
  { id: "STRONG_ACID", t: "spell", cost: 4, ench: "strongAcid", val: 99, name: "강산성비", nameJa: "強酸性雨",
    text: "영구: 상대 몬스터 전체에 카운터 2개 · 상대 몬스터가 부패로 파괴되면 상대에게 7 데미지, 낙인 카운터 1개",
    textJa: "永続: 相手モンスター全体にカウンター2個 · 相手モンスターが腐敗で破壊されると相手に7ダメージ、烙印カウンター1個" },
...
// ============================================================
export const BALANCE_VERSION = "v47"; // v47: targeted nerfs; maximum HP growth heals; Demon King locks maximum mana growth
