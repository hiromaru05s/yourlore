...
// ============================================================
// NEW CARDS 8 — 알(egg) 아키타입: 카운터 + 카운터
...
const NEW_CARDS9: CardDef[] = [
  ...
  { id: "EGG_HUNTER", t: "mon", cost: 3, atk: 0, def: 4, aura: "eggHunter", name: "에그헌터", nameJa: "エッグハンター",
    text: "이 몬스터가 '알'을 공격하면 카운터를 3 소모시킨다", textJa: "このモンスターが「卵」を攻撃するとカウンターを3消費させる" },
  { id: "INCUBATOR", t: "spell", cost: 3, act: "incubate", val: 5, name: "고급 부화기", nameJa: "高級孵化器",
    text: "자신의 '알' 1개의 카운터를 5턴 줄인다", textJa: "自分の「卵」1つのカウンターを5ターン減らす" },
  { id: "EGG_MASTER", t: "mon", cost: 3, atk: 0, def: 4, onSummon: "eggMaster", val: 1, name: "부화 마스터", nameJa: "孵化マスター",
    text: "소환시: 자신 필드의 모든 '알'의 카운터 +1", textJa: "召喚時: 自分の場の全ての「卵」のカウンター+1" },
  // ---- 피의 마법 희로애락 ----
...
const NEW_CARDS10: CardDef[] = [
  ...
  { id: "INCUBATOR_S", t: "spell", cost: 1, act: "incubate", val: 2, noShop: true, name: "부화기", nameJa: "孵化器",
    text: "자신의 '알' 1개의 카운터를 2턴 줄인다", textJa: "自分の「卵」1つのカウンターを2ターン減らす" },
...
export const PASSIVES: Record<string, { ko: PassiveDef; ja: PassiveDef; en: PassiveDef }> = {
  ...
  decay: {
    ko: { name: "부패", desc: "이 몬스터가 상대 몬스터를 공격할 때마다 카운터 1개를 부여한다. 카운터가 3개 쌓인 몬스터는 파괴되고, 그 주인은 3 데미지를 받는다." },
    ja: { name: "腐敗", desc: "このモンスターが相手モンスターを攻撃するたびカウンターを1個与える。カウンターが3個貯まったモンスターは破壊され、その持ち主は3ダメージを受ける。" },
    en: { name: "Decay", desc: "Whenever this monster attacks an enemy monster, put 1 counter on it. A monster with 3 counters is destroyed and its owner takes 3 damage." },
  },
  majesty: {
  ...
  relic: { // v40: 어튠 전용 — 컬의 세례·대숙청·용광로·통행세·은월포 등 어떤 제외 효과에도 걸리지 않는다
...
const PATCH13: Record<string, Partial<CardDef>> = {
  ...
  EGG_MASTER: { def: 2, val: 3, text: "소환시: 자신 필드의 모든 '알'의 카운터 +3", textJa: "召喚時: 自分の場の全ての「卵」のカウンター+3" },
...
const NEW_CARDS11: CardDef[] = [
  ...
  { id: "RUST_SLUG", t: "mon", cost: 3, atk: 1, def: 3, passive: ["decay"], onSummon: "decayMark", name: "러스트캡 슬러그", nameJa: "ラストキャップ・スラッグ",
    text: "부패 · 소환시: 상대 몬스터 1체에 카운터 1개를 부여", textJa: "腐敗 · 召喚時: 相手モンスター1体にカウンターを1個与える" },
...
for (const id29 of Object.keys(PATCH29)) { if (DB[id29]) Object.assign(DB[id29], PATCH29[id29]); }
...
for (const c of NEW_CARDS12) { DB[c.id] = c; }
...
const PATCH30: Record<string, Partial<CardDef>> = {
  ...
  GT11_0: { cost: 5, react: "infoDealer", val: undefined,
    text: "공격 무효 + 자신에게 1 데미지 · 첫 발동시 주사위를 굴려 나온 수만큼 카운터를 얻고 필드에 남는다",
    textJa: "攻撃無効 + 自分に1ダメージ · 初回発動時のダイスの目だけカウンターを得て場に残る" },
...
const PATCH31: Record<string, Partial<CardDef>> = {
  T8: { react: "decaytrap", val: undefined,
    text: "공격 몬스터에게 카운터 2개를 부여한다",
    textJa: "攻撃モンスターにカウンターを2個与える" },
...
// ============================================================
// NEW CARDS 13 (v31) — 카운터(상회) + 와인 아키타입
// 상회: 자신의 턴마다 카운터 적립, 20개마다 '암상인'(전 카드 풀 구매권) 지급.
// 양조: 패의 포도를 카운터로 바꾸고, 6턴 뒤 카운터만큼 '와인' 토큰 지급.
// ============================================================
const NEW_STARTERS31: CardDef[] = [
  { id: "GUILD_CO", t: "spell", cost: 2, ench: "guild", val: 99, noShop: true, name: "상회", nameJa: "商会",
    text: "자신의 턴마다 카운터 +1 · 20개가 모이면 '암상인'을 패에 넣는다 · 중복 발동 불가",
    textJa: "自分のターンごとにカウンター+1 · 20個で「闇商人」を手札に加える · 重複発動不可" },
  { id: "SLUM", t: "spell", cost: 2, noShop: true, name: "슬럼가", nameJa: "スラム街",
    text: "주사위를 굴려 나온 수만큼 자신의 '상회'에 카운터 부여",
    textJa: "ダイスを振り、出た目だけ自分の「商会」にカウンターを付与" },
  ...
  { id: "BREWING", t: "spell", cost: 3, ench: "brewing", val: 6, noShop: true, name: "양조", nameJa: "醸造",
    text: "자신의 6턴 동안 턴 시작시 패의 포도류가 카운터로(포도 1·고급 3) · 종료시 카운터만큼 '와인'을 패에 넣는다",
    textJa: "自分の6ターンの間 ターン開始時に手札のぶどうがカウンターに(ぶどう1·高級3) · 終了時カウンターの数だけ「ワイン」を手札に" },
];
const NEW_CARDS13: CardDef[] = [
  { id: "MERCH1", t: "mon", cost: 1, atk: 1, def: 2, onSummon: "guildCnt", val: 3, name: "견습 상인", nameJa: "見習い商人",
    text: "소환시: 자신의 '상회'에 카운터 3개 부여", textJa: "召喚時: 自分の「商会」にカウンターを3個付与" },
  { id: "MERCH2", t: "mon", cost: 3, atk: 1, def: 5, onSummon: "guildCnt", val: 8, name: "왕도의 상인", nameJa: "王都の商売人",
    text: "소환시: 자신의 '상회'에 카운터 8개 부여", textJa: "召喚時: 自分の「商会」にカウンターを8個付与" },
...
for (const id of DELETE_IDS34) { delete DB[id]; RANDOM_CARDS.delete(id); }

const PATCH34: Record<string, Partial<CardDef>> = {
  // ---- 단순 코스트/수치 버프 ----
  AMA: { cost: 1 },
  KIN_CALL: { cost: 2 },
  FURNACE: { cost: 2 },
  ...
  INCUBATOR: { play: 1, text: "자신의 '알' 1개의 카운터를 5턴 줄인다 (시전 1)", textJa: "自分の「卵」1つのカウンターを5ターン減らす (発動1)" },
  ...
  PURGE_TOUCH: { text: "묘지에서 카드 1장을 골라 게임에서 제외 + 카드 1장 드로우 · 자신의 카운터를 모두 제거 (시전 1)", textJa: "墓地からカード1枚を選びゲームから除外 + カード1枚ドロー · 自分のカウンターを全て取り除く (発動1)" },
  ...
  MEDITATE: { text: "자신 최대 마나 11 이하일 때만 발동 · 자신의 체력을 모두 회복하고 자신에게 카운터 1개", textJa: "自分の最大マナ11以下の時のみ発動 · 自分の体力を全回復し自分にカウンター1個" },
  MASSACRE: { text: "자신의 최대 마나 -1 · 상대 몬스터를 전부 파괴", textJa: "自分の最大マナ-1 · 相手のモンスターを全て破壊" },
  S12: { act: undefined, val: undefined, val2: undefined, text: "주사위를 굴려 5 이상이면 상대에게 카운터 1개 부여", textJa: "ダイスを振り5以上なら相手にカウンター1個を付与" },
  ...
  GS6_4: { play: 3, act: undefined, val: undefined, text: "상대에게 카운터가 있으면 카운터 3개를 부여 (시전 3)",
    textJa: "相手にカウンターがあればカウンターを3個付与 (発動3)" },
...
for (const id of Object.keys(PATCH34)) { if (DB[id]) Object.assign(DB[id], PATCH34[id]); }
RANDOM_CARDS.add("S1"); RANDOM_CARDS.add("S12"); RANDOM_CARDS.add("SX2"); RANDOM_CARDS.add("GS6_4");

// ---- 신규 스타터: 카지노 (카운터 → 카지노 주사위) ----
const NEW_CARDS15B: CardDef[] = [
  { id: "CASINO", t: "mon", cost: 3, atk: 0, def: 6, aura: "casino", noShop: true, name: "카지노", nameJa: "カジノ",
    text: "주사위를 굴릴 때마다 카운터 +1, 12개면 카지노 주사위 — ①② 자신 10뎀 / ③④ 상대 30뎀 / ⑤⑥ 상대 최대 마나 3",
    textJa: "ダイスを振るたびカウンター+1、12個でカジノダイス — ①② 自分10 / ③④ 相手30 / ⑤⑥ 相手の最大マナ3" },
];
...
// ============================================================
const PATCH36: Record<string, Partial<CardDef>> = {
  GOLEM1: { atk: 1 },
  ...
  CASINO: { def: 4,
    text: "주사위를 굴릴 때마다 카운터 +1, 12개면 카지노 주사위 — ①② 자신 30뎀 / ③④ 상대 30뎀 / ⑤ 상대 40뎀 / ⑥ 상대 최대 마나 3",
    textJa: "ダイスを振るたびカウンター+1、12個でカジノダイス — ①② 自分30 / ③④ 相手30 / ⑤ 相手40 / ⑥ 相手の最大マナ3" },
  EGG_HUNTER: { atk: 2, def: 3, val: 6, text: "상시: 이 몬스터가 '알'을 공격하면 카운터를 6 소모시킨다", textJa: "常時: このモンスターが「卵」を攻撃するとカウンターを6消費させる" },
  EGG_MASTER: { atk: 0, def: 6, val: 5, text: "소환시: 자신 필드의 모든 '알'의 카운터 +5", textJa: "召喚時: 自分の場の全ての「卵」のカウンター+5" },
  GOLEM2: { atk: 2, def: 6, aura: "leaderGolem", text: "상시: 자신 필드의 몬스터가 쓰러질 때마다 이 몬스터에 카운터 1개", textJa: "常時: 自分の場のモンスターが倒れるたびこのモンスターにカウンター1個" },
  ...
  M8: { atk: 3, def: 5 },
  M9: { onSummon: "refreshToken", text: "소환시: 카운터 1개 · 자신의 턴에 카운터를 소모해 마나 없이 제시를 갱신할 수 있다", textJa: "召喚時: カウンター1個 · 自分のターンにカウンターを消費しマナなしで提示を更新できる" },
  NGA3: { name: "골램 특공부대", nameJa: "ゴーレム特攻部隊", atk: 4, def: 1, passive: ["guts"], onSummon: "golemSquad", val: undefined,
    text: "소환시: 자신 필드에 다른 '골램' 계열 몬스터가 있으면 이 몬스터에 카운터 3개", textJa: "召喚時: 自分の場に他の「ゴーレム」系モンスターがいればこのモンスターにカウンター3個" },
  ...
  RUST_SLUG: { atk: 1, def: 4, onSummon: "decayAll", text: "소환시: 상대 몬스터 전체에 카운터 1개 · 부패로 상대 몬스터를 파괴하면 최대 마나 +1, 자신 최대 체력 +5",
    textJa: "召喚時: 相手モンスター全体にカウンター1個 · 腐敗で相手モンスターを破壊すると最大マナ+1、自分の最大体力+5" },
  ...
  TGE4: { name: "시초의 재판관", nameJa: "始原の裁判官", atk: 3, def: 4, onSummon: "originArbiter", val: undefined,
    text: "[시초] 소환시: 게임 중 1회, 덱 구성의 '시초' 카드 1장당 상대에게 카운터 1개", textJa: "[始原] 召喚時: ゲーム中1回、デッキ構成の「始原」カード1枚につき相手にカウンター1個" },
  ...
  ASSASSIN4: { atk: 23, def: 20, passive: ["majesty", "aura", "trapmaster"], summonReq: "assassinTrio", onSummon: "nightlord",
    text: "【조건】덱 구성에 이 몬스터 외 '암살자' 3종 이상 · 【소환시】상대에게 카운터 3개, 상대 세트 함정 전부 파괴",
    textJa: "【条件】デッキ構成にこのモンスター以外の「アサシン」3種以上 · 【召喚時】相手にカウンター3個、相手のセット罠を全て破壊" },
  GAMBLE: { text: "주사위 10회 — ①② 자신 8뎀 / ③④ 상대 5뎀 / ⑤ 마나 골렘 소환 / ⑥ 골램 특공부대 3체 소환",
    textJa: "ダイス10回 — ①② 自分8 / ③④ 相手5 / ⑤ マナゴーレム召喚 / ⑥ ゴーレム特攻部隊3体召喚" },
};
for (const id of Object.keys(PATCH36)) { if (DB[id]) Object.assign(DB[id], PATCH36[id]); }
...
for (const id of DELETE_IDS36) { delete DB[id]; }

const NEW_CARDS36: CardDef[] = [
  { id: "GUILD_HQ", t: "mon", cost: 6, atk: 0, def: 12, aura: "assassinHQ", turnFx: "nightMarket", name: "암살자 길드 본부", nameJa: "アサシンギルド本部",
    text: "【상시】'암살자'가 상대를 때릴 때마다 상대에게 카운터 1개 · 【매턴】'암살자' 카드를 파는 나이트 마켓 개장",
    textJa: "【常時】「アサシン」が相手を叩くたび相手にカウンター1個 · 【毎ターン】「アサシン」カードを売るナイトマーケット開店" },
  { id: "WORLD_TREE", t: "mon", cost: 10, atk: 0, def: 25, passive: ["aura", "guts"], turnFx: "worldTree", name: "세계수", nameJa: "世界樹",
    text: "【상시】자신 최대 체력이 늘면 카운터 1개 · 【매턴】카운터 1개로 자신 몬스터 전체 전회복, 자신 체력을 80%로",
    textJa: "【常時】自分の最大体力が増えるとカウンター1個 · 【毎ターン】カウンター1個で自分のモンスター全体を全回復、自分の体力を80%に" },
  // ---- 토큰 (구매 불가) ----
  ...
  { id: "ORIGIN_RITE", t: "spell", cost: 0, noShop: true, ench: "originRite", val: 99, exileOnDestroy: true, name: "시초의 술식", nameJa: "始原の術式",
    text: "영구: 수호자 외 시초 몬스터 소환 시 상대 필드 카드 1장 파괴 · 없으면 상대에게 카운터 1개",
    textJa: "永続: 守護者以外の始原モンスター召喚時、相手の場のカード1枚を破壊 · なければ相手にカウンター1個" },
...
for (const c of NEW_CARDS36) { DB[c.id] = c; }

// ============================================================
// BALANCE PATCH 37 (v37) — 성(城) 아키타입 + 함정 전면 리워크 + 마법 조정 (유저 스펙, 2026-09-04)
// ============================================================
const PATCH37: Record<string, Partial<CardDef>> = {
  // ---- v36 후속 조정 ----
  ...
  T8: { react: "caltrops", text: "공격 몬스터를 포함한 상대 몬스터 최대 3체에 카운터 2개씩", textJa: "攻撃モンスターを含む相手モンスター最大3体にカウンター2個ずつ" },
  ...
  T12: { react: "rampart", val: undefined, text: "코스트 5 이하 몬스터의 공격에만 발동 · 공격 무효 + 그 몬스터를 패로 · 자신 '성'에 카운터 5개",
    textJa: "コスト5以下のモンスターの攻撃にのみ発動 · 攻撃無効 + そのモンスターを手札に戻す · 自分の「城」にカウンター5個" },
  ...
  T4: { react: "magicCounter", val: undefined, val2: undefined, text: "공격 무효 · 상대에게 카운터 1개", textJa: "攻撃無効 · 相手にカウンター1個" },
  ...
  GT5_1: { react: "lavaPit", text: "공격 무효 + 공격 몬스터 파괴 · 그 공격력이 4 이상이면 상대에게 카운터 1개", textJa: "攻撃無効 + 攻撃モンスターを破壊 · その攻撃力が4以上なら相手にカウンター1個" },
...
for (const id of Object.keys(PATCH37)) { if (DB[id]) Object.assign(DB[id], PATCH37[id]); }

const DELETE_IDS37 = ["GT5_2", "GT5_3", "GT6_0", "GT6_2", "GT6_3", "GT6_5", "GT8_1", "GT8_2", "GT8_3", "GT8_5", "GT9_3", "GT10_1", "GT12_1", "NT_NULL8"];
for (const id of DELETE_IDS37) { delete DB[id]; RANDOM_CARDS.delete(id); }

const NEW_STARTERS37: CardDef[] = [
  { id: "CASTLE", t: "mon", cost: 2, atk: 0, def: 3, passive: ["taunt"], onSummon: "castleInit", aura: "castle", noShop: true, name: "성", nameJa: "城",
    text: "【소환시】카운터 3개 · 【상시】카운터 1개로 공격 무효 · 코스트 5 이상 소환 불가 · 병사·기사 소환 시 카운터 +1",
    textJa: "【召喚時】カウンター3個 · 【常時】カウンター1個で攻撃を無効化 · コスト5以上は召喚不可 · 兵士・騎士召喚時にカウンター+1" },
  { id: "ACID_RAIN", t: "spell", cost: 2, ench: "acidRain", val: 99, noShop: true, name: "산성비", nameJa: "酸性雨",
    text: "영구: 상대 몬스터가 부패로 파괴될 때마다 상대에게 카운터 1개", textJa: "永続: 相手モンスターが腐敗で破壊されるたび相手にカウンター1個" },
];
const NEW_CARDS37: CardDef[] = [
  ...
  { id: "EXPANSION", t: "spell", cost: 3, name: "증축", nameJa: "増設", text: "자신 필드에 '성'이 있을 때만 · 자신의 '성'에 카운터 5개", textJa: "自分の場に「城」がある時のみ · 自分の「城」にカウンター5個" },
  ...
  { id: "TREASON", t: "spell", cost: 1, name: "반역죄", nameJa: "反逆罪", text: "상대 필드에 '성'이 있을 때만 · 상대 필드의 카드 전부 파괴, 상대에게 카운터 3개", textJa: "相手の場に「城」がある時のみ · 相手の場のカードを全て破壊、相手にカウンター3個" },
  { id: "STRONG_ACID", t: "spell", cost: 4, ench: "strongAcid", val: 99, name: "강산성비", nameJa: "強酸性雨",
    text: "영구: 발동 시 상대 몬스터 전체에 카운터 2개 · 상대 몬스터가 부패로 파괴되면 상대에게 7 데미지, 낙인 1개",
    textJa: "永続: 発動時に相手モンスター全体にカウンター2個 · 相手モンスターが腐敗で破壊されると相手に7ダメージ、烙印1個" },
  { id: "ROTTEN_GROUND", t: "spell", cost: 2, ench: "rottenGround", val: 99, name: "부패한 땅", nameJa: "腐敗した土地",
    text: "영구: 필드에 소환되는 모든 몬스터에 카운터 2개", textJa: "永続: 場に召喚される全てのモンスターにカウンター2個" },
  { id: "UNBRAND", t: "spell", cost: 1, name: "제인", nameJa: "徐印", text: "상대와 자신의 카운터를 모두 제거", textJa: "相手と自分のカウンターを全て取り除く" },
...
for (const c of [...NEW_STARTERS37, ...NEW_CARDS37]) { DB[c.id] = c; }
...
const NEW_CARDS38: CardDef[] = [
  ...
  { id: "KNIGHT_TEACH", t: "spell", cost: 3, name: "골램 킹의 가르침", nameJa: "ゴーレムキングの教え",
    text: "자신 필드의 모든 몬스터에 '기합' 부여 · 이미 기합이 있으면 카운터 3개", textJa: "自分の場の全モンスターに「気合」を付与 · 既に気合があればカウンター3個" },
...
Object.assign(DB.CASTLE, { def: 2, val: 2, text: "【소환시】카운터 2개 · 【상시】카운터 1개로 공격 무효 · 코스트 5 이상 소환 불가 · 병사·기사 소환 시 카운터 +1",
  textJa: "【召喚時】カウンター2個 · 【常時】カウンター1個で攻撃を無効化 · コスト5以上は召喚不可 · 兵士・騎士召喚時にカウンター+1" });
// ---- v39: 주술사 계열 — 꼬마 주술사 → 견습 주술사(2코) + 초급/중급/상급/특급 주술사 ----
Object.assign(DB.NHEX, { cost: 2, name: "견습 주술사", nameJa: "見習い呪術師" });
const NEW_CARDS39: CardDef[] = [
...
for (const c of NEW_CARDS39) { DB[c.id] = c; }
// ---- v41: 컬 제외 아키타입 확장(선별자/콜로세움) + 낙인·부패 대응 + 무법지대 + 스타터 차원의 균열 ----
const NEW_CARDS41: CardDef[] = [
  { id: "WASH_DEVICE", t: "trap", cost: 3, play: 1, react: "washDevice", name: "세척 장치", nameJa: "洗浄装置",
    text: "상대 필드의 '부패'를 가진 몬스터를 모두 파괴 · 파괴한 수만큼 상대에게 카운터",
    textJa: "相手の場の「腐敗」を持つモンスターを全て破壊 · 破壊した数だけ相手にカウンター" },
  ...
  { id: "UNBRANDER", t: "mon", cost: 3, atk: 1, def: 3, onSummon: "unbrand", name: "제인사", nameJa: "除印師",
    text: "소환시: 자신에게 카운터가 있으면 자신의 카운터 1개를 제거",
    textJa: "召喚時: 自分にカウンターがあれば自分のカウンターを1個取り除く" },
...
for (const c of [...NEW_CARDS41, ...NEW_STARTERS41]) { DB[c.id] = c; }
DECK_POOL.push(...NEW_STARTERS41.map((c) => c.id));
// ---- v41b: 마법/함정 14종 — 코스트 0·주사위·낙인·기합·손패 시너지 ----
const NEW_CARDS41B: CardDef[] = [
  ...
  { id: "PENANCE", t: "spell", cost: 2, act: "penance", name: "고행의 대가", nameJa: "苦行の対価",
    text: "자신의 카운터 1개당 최대 마나 +2, 최대 체력 +10", textJa: "自分のカウンター1個につき最大マナ+2、最大体力+10" },
  ...
  { id: "MIND_BURST", t: "spell", cost: 2, act: "mindBurst", name: "정신 방출술", nameJa: "精神放出術",
    text: "자신 필드의 카운터를 모두 제거 · 그 수 ×4 데미지를 상대에게", textJa: "自分の場のカウンターを全て取り除く · その数×4ダメージを相手に" },
...
for (const c of NEW_CARDS41B) { DB[c.id] = c; }
...
// v31(구): // v31: 카운터(상회/슬럼가/견습·왕도 상인/암상인) + 와인 아키타입(포도/고급 포도/양조/와인)
