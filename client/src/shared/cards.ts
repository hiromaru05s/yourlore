...
const CORE: Record<string, CardDef> = {
  ...
  S12: { id: "S12", t: "spell", cost: 3, name: "강화 주문", text: "자신 몬스터 1체에 공격+2 / 체력+2(지속)", act: "buffPerm", val: 2, val2: 2 },
...
const SPELL_TPL: FxTpl[] = [
  ...
  { nouns: ["룬 각인", "축복", "룬 강화"], nounsJa: ["ルーン刻印", "祝福", "ルーン強化"], mk: (c) => ({ act: "buffPerm", val: R(c * 0.8), val2: R(c * 0.8), text: `자신 몬스터 1체 공격+${R(c * 0.8)}/체력+${R(c * 0.8)}(지속)`, textJa: `自分のモンスター1体の攻撃+${R(c * 0.8)}/体力+${R(c * 0.8)}(持続)` }) },
...
const CORE_JA: Record<string, { name: string; text?: string }> = {
  ...
  M6: { name: "ツイン・ファング", text: "召喚時: 敵モンスターの体力-2(持続)" },
  ...
  S12: { name: "強化の呪文", text: "自分のモンスター1体に攻撃+2/体力+2(持続)" },
...
const PATCH2: Record<string, Partial<CardDef>> = {
  ...
  S7: { val: 3, text: "자신 몬스터 전체 공격 +3(이번 턴) + 자신 최대 체력 +2", textJa: "自分のモンスター全体の攻撃+3(このターン) + 自分の最大体力+2" },
  S8: { text: "묘지에서 원하는 1장을 패로", textJa: "墓地から好きな1枚を手札へ" },
  S12: { val: 2, val2: 1, text: "자신 몬스터 1체에 공격+2 / 체력+1(지속)", textJa: "自分のモンスター1体に攻撃+2/体力+1(持続)" },
  ...
  GS7_2: { text: "자신 체력 13 회복 · 이 카드 3번째 사용부터 피격 시마다 자신 체력 5 회복", textJa: "自分の体力13回復 · このカード3回目使用から被攻撃ごとに自分の体力5回復" },
...
const NEW_CARDS5: CardDef[] = [
  ...
  { id: "WORLD_HEART", t: "spell", cost: 5, play: 4, ench: "growHpMana", val: 99, val2: 7, name: "세계수의 심장", nameJa: "世界樹の心臓",
    text: "영구: 자신의 턴마다 자신 최대 체력 +7. 이 카드가 필드에 있는 한 자신의 최대 마나 -2 (시전 4)", textJa: "永続: 自分のターンごとに自分の最大体力+7。このカードが場にある限り自分の最大マナ-2 (発動4)" },
  ...
  { id: "HERMIT", t: "spell", cost: 8, play: 7, act: "heal", val: 8, name: "은둔의 안식", nameJa: "隠遁の安息",
    text: "자신의 필드에 몬스터가 없는 경우에만 발동 가능. 자신 체력 완전 회복 + 자신 최대 체력 +15 (시전 7)", textJa: "自分の場にモンスターがいない場合のみ発動可能。自分の体力全回復 + 自分の最大体力+15 (発動7)" },
...
const NEW_CARDS6: CardDef[] = [
  ...
  { id: "ELITE", t: "mon", cost: 4, atk: 2, def: 4, onSummon: "eliteBuff", name: "정예 기사단장", nameJa: "精鋭騎士団長",
    text: "소환시: 자신의 덱+묘지가 8장 이하면 이 몬스터의 공격 +4(지속)", textJa: "召喚時: 自分のデッキ+墓地が8枚以下ならこのモンスターの攻撃+4(持続)" },
...
const NEW_CARDS9: CardDef[] = [
  ...
  { id: "VAMP_BUTLER", t: "mon", cost: 2, atk: 1, def: 4, aura: "vampButler", name: "뱀파이어 집사", nameJa: "ヴァンパイア執事",
    text: "상시: 이 몬스터가 상대 몬스터를 공격할 때마다 흡혈 카운트 +1 · 3카운트마다 '견습 흡혈귀'를 자신 필드에 소환 · 이 몬스터는 '흡혈귀' 계열로 취급",
    textJa: "常時: このモンスターが相手モンスターを攻撃するたび吸血カウント+1 · 3カウントごとに「見習い吸血鬼」を自分の場に召喚 · このカードは「吸血鬼」系列として扱う" },
  { id: "BLOOD_SECRET", t: "spell", cost: 2, name: "피의 마법 - 비술", nameJa: "血の魔法 - 秘術",
    text: "자신에게 9 데미지. 자신 필드의 '흡혈귀' 계열 몬스터 1체를 파괴 — 파괴 성공 시 최대 마나 +3, 자신 최대 체력 +10",
    textJa: "自分に9ダメージ。自分の場の「吸血鬼」系列モンスター1体を破壊 — 破壊成功時、最大マナ+3、自分の最大体力+10" },
...
const NEW_CARDS10: CardDef[] = [
  ...
  { id: "GUILD_HALL", t: "mon", cost: 2, atk: 0, def: 2, aura: "assassinGuild", noShop: true, name: "암살자 길드", nameJa: "アサシンギルド",
    text: "상시: '암살자' 몬스터 또는 이 몬스터로 상대에게 데미지를 줄 때마다 카운트 +1 · 3카운트 달성 시 상대에게 14 데미지 (카운트 초기화) · 파괴되면 카운트도 사라진다",
    textJa: "常時: 「アサシン」モンスターかこのモンスターで相手にダメージを与えるたびカウント+1 · 3カウント達成で相手に14ダメージ (カウントリセット) · 破壊されるとカウントも消える" },
...
const PATCH13: Record<string, Partial<CardDef>> = {
  ...
  VAMP4: { passive: ["void"], text: "'피의 마법' 발동 시: 특급 흡혈귀를 자신 필드에 소환 (1회) · 상대에게 입힌 데미지의 50%만큼 자신 최대 체력 획득 · 공허", textJa: "「血の魔法」発動時: 特級吸血鬼を自分の場に召喚 (1回) · 相手に与えたダメージの50%だけ自分の最大体力を得る · 虚無" },
  ...
  DRAGON_EGG: { hatchDur: 6, aura: "ward",
    text: "상시: 공격 불가 · 부화 8턴(양측 턴 포함) / 내구도 6 · 상대 몬스터의 공격은 내구도만 1 소모 · 내구도가 남은 채 부화 완료 시 흑룡·적룡·청룡 중 1체 소환 (소환 4)",
    textJa: "常時: 攻撃不可 · 孵化8ターン(両者のターン込み) / 耐久6 · 敵モンスターの攻撃は耐久のみ1消費 · 耐久が残ったまま孵化完了で黒竜・赤竜・青竜から1体召喚 (召喚4)" },
  BEAST_EGG: { hatchDur: 7, aura: "ward",
    text: "상시: 공격 불가 · 부화 10턴(양측 턴 포함) / 내구도 7 · 상대 몬스터의 공격은 내구도만 1 소모 · 내구도가 남은 채 부화 완료 시 신수 소환 (소환 5)",
    textJa: "常時: 攻撃不可 · 孵化10ターン(両者のターン込み) / 耐久7 · 敵モンスターの攻撃は耐久のみ1消費 · 耐久が残ったまま孵化完了で神獣を召喚 (召喚5)" },
...
const NEW_STARTERS15: CardDef[] = [
  { id: "GAMBLER", t: "mon", cost: 3, atk: 0, def: 1, turnFx: "gambler", noShop: true, name: "도박꾼", nameJa: "ギャンブラー",
    text: "자신의 턴 시작시: 주사위를 굴려 4·5·6이면 최대 마나 +1, 자신 최대 체력 +5", textJa: "自分のターン開始時: ダイスを振り4・5・6なら最大マナ+1、自分の最大体力+5" },
...
const NEW_CARDS15: CardDef[] = [
  ...
  { id: "ELDER_ELF_KING", t: "mon", cost: 7, atk: 2, def: 8, onSummon: "elderKing", summonReq: "elderKing", name: "엘더 하이엘프 킹", nameJa: "エルダーハイエルフキング",
    text: "묘지에 '하이엘프'가 있고 자신의 최대 체력이 99 이상일 때만 소환 가능 · 소환시: 하이엘프 2체를 자신 필드에 소환 후, 자신 필드의 모든 '하이엘프'의 공격 +15(지속)",
    textJa: "墓地に「ハイエルフ」があり自分の最大体力が99以上の時のみ召喚可能 · 召喚時: ハイエルフ2体を自分の場に召喚し、自分の場の全ての「ハイエルフ」の攻撃+15(持続)" },
];
...
const DICE_TEXTS: Record<string, { ko: string; ja: string }> = {
  ...
  GS8_5: { ko: "아군 전체 공격 +7(이번 턴) · 주사위 6이면 코스트 6 이하 몬스터 무작위 소환", ja: "味方全体の攻撃+7(このターン) · ダイス6でコスト6以下のモンスターをランダム召喚" },
...
// 황금상자: 1d6 표
CHEST_ODDS.ko = { title: "황금상자 주사위", rows: ["1 — 꽝: 상대 필드에 미믹(3/2)", "2·3 — 자신 체력 +3", "4·5 — 최대 마나 +1", "6 — 자신 최대 체력 +5"] };
CHEST_ODDS.ja = { title: "宝箱のダイス", rows: ["1 — ハズレ: 相手の場にミミック(3/2)", "2·3 — 自分の体力 +3", "4·5 — 最大マナ +1", "6 — 自分の最大体力 +5"] };
CHEST_ODDS.en = { title: "Golden chest die", rows: ["1 — Dud: Mimic (3/2) on enemy field", "2·3 — Your HP +3", "4·5 — Max mana +1", "6 — Your max HP +5"] };
...
const PATCH29: Record<string, Partial<CardDef>> = {
  ...
  GS8_4: { text: "아군 전체 공격 +13(이번 턴) · 공격 +2(지속)", textJa: "味方全体の攻撃+13(このターン) · 攻撃+2(持続)" },
  // 악마의 주사위 ③④: 상대 '최대' 마나 -1
  DICE8: { text: "주사위 1~6 — ①② 자신 최대마나-4 / ③④ 상대 최대마나-1·14뎀 / ⑤ 폭풍의 전사(2회공격) 소환 / ⑥ 상대 마법·함정 전멸+폭풍의 전사 2체+최대마나+2+자신 체력+10",
    textJa: "ダイス1~6 — ①② 自分の最大マナ-4 / ③④ 相手の最大マナ-1・14 / ⑤ 嵐の戦士(2回攻撃)召喚 / ⑥ 相手の魔法・罠全滅+嵐の戦士2体+最大マナ+2+自分の体力+10" },
  // 명상 / 성역의 기도: 엔진에 있는 "체력이 최대치의 80% 미만" 게이트가 문구에 없었음
  MEDITATE: { text: "자신 체력이 최대 체력의 80% 미만이고 이번 턴에 다른 카드를 플레이하지 않았을 때만 발동 가능. 자신 최대 체력의 80%까지 자신 체력 회복",
    textJa: "自分の体力が最大体力の80%未満で、このターンに他のカードをプレイしていない場合のみ発動可能。自分の最大体力の80%まで回復" },
  PRAYER: { text: "자신 최대 마나가 12 이하, 자신 체력이 최대 체력의 80% 미만이고 이번 턴에 다른 카드를 플레이하지 않았을 때만 발동 가능. 자신 최대 체력의 80%까지 자신 체력 회복 (시전 5)",
    textJa: "自分の最大マナが12以下、自分の体力が最大体力の80%未満で、このターンに他のカードをプレイしていない場合のみ発動可能。自分の最大体力の80%まで回復 (発動5)" },
  // 붕괴 진동: 실제 동작은 "최대 체력을 1로 + 누적 데미지 초기화 + 알 제외"
  SHATTER: { text: "자신에게 5 데미지. 양 필드의 모든 몬스터의 최대 체력이 1이 된다(지속 · 누적 데미지 초기화 · 알 제외) (시전 2)",
    textJa: "自分に5ダメージ。両方の場の全モンスターの最大体力が1になる(持続 · 蓄積ダメージはリセット · 卵を除く) (発動2)" },
  // 트릭룸: 스왑되는 값은 최대 체력이고 누적 데미지는 유지 → 스왑 즉시 파괴될 수 있다
  TRICKROOM: { text: "2턴 동안 필드 위 모든 몬스터의 공격력과 최대 체력이 반전된다(누적 데미지는 유지 — 최대 체력이 누적 데미지 이하가 되면 파괴) · 반전 중에는 체력이 오르는 효과가 공격력을, 공격력이 오르는 효과가 체력을 올린다 · 효과가 끝나면 반전 중에 오른 스탯은 계승된다",
    textJa: "2ターンの間、場の全モンスターの攻撃力と最大体力が反転する(蓄積ダメージは維持 — 最大体力が蓄積ダメージ以下になると破壊) · 反転中は体力が上がる効果は攻撃力を、攻撃力が上がる効果は体力を上げる · 効果終了後、反転中に上がったステータスは継承される" },
...
const NEW_CARDS12: CardDef[] = [
  ...
  { id: "HPS_GRAFT", t: "spell", cost: 2, act: "buffPerm", val: -2, val2: 6, name: "생명 접목", nameJa: "生命の接ぎ木",
    text: "자신 몬스터 1체의 체력 +6(지속) · 그 몬스터의 공격 -2(지속)", textJa: "自分のモンスター1体の体力+6(持続) · そのモンスターの攻撃-2(持続)" },
