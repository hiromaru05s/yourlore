...
const CORE: Record<string, CardDef> = {
  ...
  M6: { id: "M6", t: "mon", cost: 2, name: "트윈 팽", atk: 4, def: 3, text: "소환시: 적 몬스터 체력 -2(지속)", onSummon: "defDown", val: 2 },
...
const MON_TPL: MonTpl[] = [
  ...
  { nouns: ["주술사", "저주술사", "마녀"], nounsJa: ["呪術師", "呪詛師", "魔女"], atk: (c) => R(c * 1.2), def: (c) => R(c * 0.9), mk: (c) => ({ onSummon: "defDown", val: R(c * 0.8), text: `소환시: 적 몬스터 체력 -${R(c * 0.8)}(지속)`, textJa: `召喚時: 敵モンスターの体力-${R(c * 0.8)}(持続)` }) },
...
const PATCH: Record<string, Partial<CardDef>> = {
  ...
  GT8_0: { play: 7, react: "guardbuff", val: 4, val2: 1, text: "공격 무효 + 자신 몬스터 전체 체력 +4 + 1장 드로우", textJa: "攻撃無効 + 自分のモンスター全体の体力+4 + 1枚ドロー" },
  GT8_1: { react: "guardEnemyDef", val: 4, text: "공격 무효 + 상대 몬스터 전체 체력 -4", textJa: "攻撃無効 + 敵モンスター全体の体力-4" },
...
const PATCH2: Record<string, Partial<CardDef>> = {
  ...
  GS7_2: { text: "자신 체력 13 회복 · 이 카드 3번째 사용부터 피격 시마다 자신 체력 +5", textJa: "自分の体力13回復 · このカード3回目使用から被攻撃ごとに自分の体力+5" },
...
const PATCH4: Record<string, Partial<CardDef>> = {
  ...
  GT8_0: { text: "공격 무효 + 자신 몬스터 전체 체력 +4 + 1장 드로우 (시전 7)", textJa: "攻撃無効 + 自分のモンスター全体の体力+4 + 1枚ドロー (発動7)" },
...
const PATCH5: Record<string, Partial<CardDef>> = {
  ...
  // 8코+ 시전비 대할인
  GT8_0: { play: 4, text: "공격 무효 + 자신 몬스터 전체 체력 +4 + 1장 드로우 (시전 4)", textJa: "攻撃無効 + 自分のモンスター全体の体力+4 + 1枚ドロー (発動4)" },
