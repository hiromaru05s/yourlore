...
function offerChosenMage(g: GameState, ctx: Ctx): void {
  ...
  g.pending = {
    ...
    hint: "선택받은 마법사 — 발동할 마법사 선택 (제외된 컬 1장 → 묘지, 상대에게 6 데미지 · 취소 가능)",
    hintJa: "選ばれし魔法使い — 発動する魔法使いを選択 (除外されたカル1枚→墓地, 相手に6ダメージ · キャンセル可)",
  };
...
function tickTurnFx(g: GameState, ctx: Ctx, p: PlayerState): void {
  ...
  for (const m of [...p.field]) {
    ...
    switch (m.turnFx) {
      ...
      case "legendGambler": { // 전설의 도박꾼: 주사위 6 → 최대 마나 +10 · 덱/묘지에 '도박꾼'이 있으면 총 3회 굴림
        const rolls = [...p.deck, ...p.discard].some((c) => (c.name || "").includes("도박꾼")) ? 3 : 1;
        const lgRolls: number[] = [];
        for (let i = 0; i < rolls; i++) lgRolls.push(randInt(g, 6) + 1);
        ctx.ev.push({ type: "dice", player: side(g, p), rolls: lgRolls, success: lgRolls.includes(6) });
        for (let i = 0; i < rolls; i++) {
          const r = lgRolls[i];
          if (r === 6) { p.maxMana += 10; ctx.log(`  └ ${cn(m)} 🎲 ${r} → <span class="good">잭팟! 최대 마나 +10 (${p.maxMana})</span>`, `  └ ${cn(m)} 🎲 ${r} → <span class="good">ジャックポット！最大マナ+10 (${p.maxMana})</span>`); }
          else ctx.log(`  └ ${cn(m)} 🎲 ${r}`, `  └ ${cn(m)} 🎲 ${r}`);
        }
        break;
      }
...
function resolveAttackCore(g: GameState, ctx: Ctx, att: FieldMon, targetUid: string | null): void {
  ...
  if ((tc = takeTrap(g, ctx, o, "bulwark"))) {
    ...
    ctx.log(
      `  └ <span class="dmg">함정 ${cn(tc)}!</span> 공격 무효 + ${cn(att)} 최대 체력 -${tc.val}`,
      `  └ <span class="dmg">トラップ ${cn(tc)}!</span> 攻撃無効 + ${cn(att)} 最大体力 -${tc.val}`,
    );
    recheckDeaths(g, ctx);
    return;
  }
  ...
  if ((tc = takeTrap(g, ctx, o, "guardEnemyDef"))) { // GT8_1: negate + all attacker-side def down
    ...
    ctx.log(`  └ <span class="dmg">함정 ${cn(tc)}!</span> 공격 무효 + 상대 몬스터 전체 최대 체력 -${tv}`, `  └ <span class="dmg">トラップ ${cn(tc)}!</span> 攻撃無効 + 敵モンスター全体の最大体力-${tv}`);
    recheckDeaths(g, ctx);
    return;
  }
...
function customSpell(g: GameState, ctx: Ctx, card: CardInst): void {
  ...
  switch (card.id) {
    ...
    case "WALLBREAK1": case "SNIPE1": { // 조건부 단일 제거: 조건 충족 몬스터 중 가장 가치 높은 것
      ...
      const lim = 1;
      const ts = o.field.filter((mm) => (isAtk ? effAtk(o, mm) : curHp(o, mm)) <= lim)
        .sort((a2, b2) => (effAtk(o, b2) + effDef(o, b2)) - (effAtk(o, a2) + effDef(o, a2)));
      if (ts.length) {
        ctx.log(`${tag(p, card)} ${isAtk ? "공격력" : "체력"} ${lim} 이하 몬스터 ${cn(ts[0])} 파괴`, `${tag(p, card)} ${isAtk ? "攻撃力" : "体力"}${lim}以下のモンスター ${cn(ts[0])} を破壊`);
...
export function summonReqMet(p: PlayerState, card: CardInst): boolean {
  ...
  if (card.summonReq === "elderKing") return p.maxHp >= 99 && p.discard.some((c) => c.id === "HIGH_ELF");
  return true;
}
...
function playFromHand(g: GameState, ctx: Ctx, idx: number): void {
  ...
  if (card.t === "mon") {
    ...
    summonMonster(g, ctx, p, card); return;
  }
  if (card.t === "spell") {
    ...
    if (card.id === "SNIPE1" && !o0.field.some((m) => curHp(o0, m) <= 1)) { ctx.log("  └ 체력 1 이하 적 몬스터가 없습니다", "  └ 体力1以下の敵モンスターがいません"); return; }
    ...
    if (card.id === "GOLIATH_HUNT" && !o0.field.some((m) => effDef(o0, m) >= 20)) { ctx.log("  └ 최대 체력 20 이상 적 몬스터가 없습니다", "  └ 最大体力20以上の敵モンスターがいません"); return; }
...
function resolveTarget(g: GameState, ctx: Ctx, uid: string | null): void {
  ...
  const p = g.players[g.cur];
  const o = g.players[1 - g.cur];
  const d = (pending.data || {}) as { val?: number; val2?: number; attackerUid?: string; count?: number; exclude?: string; sourceId?: string; zone?: string; fired?: string[]; noMon?: boolean; anySide?: boolean; trapOnly?: boolean; enchOnly?: boolean };
  ...
  if (pending.kind === "oppMon") {
    ...
    if (pending.reason === "defDown" || pending.reason === "weaken") { tm.defMod = (tm.defMod || 0) - (d.val || 0); ctx.log(`  └ ${cn(tm)} 의 최대 체력 -${d.val}`, `  └ ${cn(tm)} の最大体力 -${d.val}`); recheckDeaths(g, ctx); }
    ...
    if (pending.reason === "buffTurn") {
      ...
      if (ci >= 0) {
        ...
        p.discard.push(c);
        ctx.log(`<span class="t">${cn(tm)}</span> 발동 — 제외된 컬 1장을 묘지로 되돌리고 상대에게 6 데미지`, `<span class="t">${cn(tm)}</span> 発動 — 除外されたカル1枚を墓地に戻し相手に6ダメージ`);
        ctx.dealDamage(o, 6, cn(tm), cn(tm));
      }
    ...
    if (i >= 0) {
      ...
      o.discard.push(c);
      ctx.log(`<span class="t">${p.name}</span> ${cn(c)} 을(를) 상대 묘지로 되돌림`, `<span class="t">${p.name}</span> ${cn(c)} を相手の墓地に戻す`);
      const remaining = ((d.val as number) ?? 1) - 1;
