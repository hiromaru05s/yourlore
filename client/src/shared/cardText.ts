...
const firstSegTagged = (s: string): boolean => s.split(" · ")[0].includes("【");
...
function canon(s: string, lang: "ko" | "ja" | "en"): string {
...
function hoistTag(s: string): string {
  for (const tag of HOIST_TAGS) {
    ...
    if (i > 0 && !pre.includes("【") && !/\s·\s/.test(pre) && !/^\s*\[[^\]]+\]\s*$/.test(pre)) return tag + pre.trim() + " " + s.slice(i + tag.length).trim();
  }
  return s;
}
  ...
  for (const pool of pools) {
    for (const id of Object.keys(pool)) {
      ...
      const fin = (txt: string, lang: "ko" | "ja" | "en"): string =>
        unifySeparators(stripCostPrefix(stripCastCost(canon(hoistTag(txt), lang))));
      // 바닐라 몬스터("—")는 세 언어 모두 같은 표시 — ja/en 에 undefined 가 새지 않게
      if (c.text === "—") { c.textJa ??= "—"; c.textEn ??= "—"; }
      const pre = (txt: string, lang: "ko" | "ja" | "en"): string => stripKeywords(c, unifySeparators(txt), lang, keywordNames);
      if (c.text && c.text !== "—") c.text = fin(ov?.ko ?? stdKo(c, pre(c.text, "ko")), "ko");
      if (c.textJa && c.textJa !== "—") c.textJa = fin(ov?.ja ?? stdJa(c, pre(c.textJa, "ja")), "ja");
      if (c.textEn && c.textEn !== "—") c.textEn = fin(c.textEn === ko0 ? c.text : ov?.en ?? stdEn(c, pre(c.textEn, "en")), "en");
    }
  }
}
