...
export function prepareDuel(root:HTMLElement,mount:Promise<void>):void{
  ...
  })().finally(()=>{root.classList.remove('duel-preparing');root.removeAttribute('aria-busy');});
...
export function warmDuel():Promise<void>{
  return warming??=(async()=>{
    ...
    await Promise.allSettled([...urls.map(async url=>{const r=await fetch(url);if(r.ok)await r.arrayBuffer();}),...coinImages.map(decode),import('./duelScene')]);
  })();
}
