...
export function foldIntoRift(point:RiftPoint,row:RiftPoint,target:RiftPoint,q:number,t:number,width:number):RiftPoint {
  ...
  const wave=Math.sin(Math.PI*u)*Math.sin(q*Math.PI*2-u*Math.PI)*width*.32;
  const twist=Math.sin(Math.PI*u)*Math.sin(q*Math.PI+u*2)*.85;
  ...
  return {
    ...
    z:(point.z*remain)+Math.sin(Math.PI*u)*Math.sin(q*Math.PI*2+u*4)*width*.28,
  };
}
