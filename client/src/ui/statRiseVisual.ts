...
export function createStatRiseVisual(kind: 'attack' | 'health' = 'attack') {
...
const surfaceMaterial=shader(`varying vec2 vUv;uniform float alpha;uniform vec3 tint;float box(vec2 p,vec2 b){vec2 q=abs(p)-b;return length(max(q,0.))+min(max(q.x,q.y),0.);}void main(){vec2 p=(vUv-.5)*vec2(1.,1.56);float d=box(p,vec2(.465,.737));float rim=1.-smoothstep(.003,.012,abs(d));gl_FragColor=vec4(tint,rim*alpha);}`,{alpha:{value:0},tint:{value:new T.Color(health?0xe86a75:0x599deb)}});
...
const particles=Array.from({length:6},(_,i)=>{const material=new T.MeshBasicMaterial({color:health?0xe8a0a6:0x9bc8f4,transparent:true,side:T.DoubleSide,depthWrite:false});resources.push(material);const o=mesh(shardGeometry(),material);return{o,material,x:i%2? .49:-.49,z:[-.62,.06,.62][Math.floor(i/2)],delay:(i%3)*.035,size:.025};});
