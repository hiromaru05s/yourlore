...
export function landingQuaternion(value:number):T.Quaternion {
  const alignment=new T.Quaternion().setFromUnitVectors(FACE_NORMALS[value],new T.Vector3(0,0,1));
  return new T.Quaternion().setFromEuler(new T.Euler(.19,-.23,.035)).multiply(alignment);
}
...
export async function mountDiceScene(host:HTMLElement,rolls:number[],casino:boolean,signal:AbortSignal):Promise<DiceScene|null> {
  ...
  function dispose(){
    ...
    scene.traverse(o=>{if(o instanceof T.Mesh){geo.add(o.geometry);(Array.isArray(o.material)?o.material:[o.material]).forEach(m=>mats.add(m));}});
    geo.forEach(g=>g.dispose());mats.forEach(m=>m.dispose());texture?.dispose();environment?.dispose();renderer.dispose();renderer.forceContextLoss();canvas.remove();host.classList.remove('d3-gpu');
  }
  ...
  try{
    ...
    scene.add(new T.HemisphereLight(0xd9ecff,0x323844,2.1));
    const key=new T.DirectionalLight(0xffefcf,3);key.position.set(-3,5,6);scene.add(key);
    const rim=new T.DirectionalLight(0x93d9ff,1.3);rim.position.set(4,2,-2);scene.add(rim);
    const dice=rolls.map(value=>{const die=createDie(texture,casino);scene.add(die);return {die,end:landingQuaternion(value)};});
    host.append(canvas);host.classList.add('d3-gpu');
    const camera=new T.PerspectiveCamera(30,1,.1,50);camera.position.set(0,0,5.4);
    const rect=host.getBoundingClientRect();renderer.setSize(rect.width,rect.height,false);camera.aspect=rect.width/rect.height;
    camera.position.z=Math.max(5.4,(rolls.length*1.35+.5)/(2*Math.tan(Math.PI/12)*camera.aspect));camera.updateProjectionMatrix();
    ...
    const tick=(now:number)=>{
      ...
      let settled=true;
      dice.forEach(({die,end},i)=>{
        const t=Math.max(0,Math.min(1,(now-start-i*95)/1120));if(t<1)settled=false;
        const remaining=Math.pow(1-t,2.25);
        die.quaternion.copy(new T.Quaternion().setFromEuler(new T.Euler(remaining*Math.PI*4,remaining*Math.PI*5,remaining*(i%2?-2:2)))).multiply(end);
        const drop=t<.6?Math.pow(1-t/.6,2)*3.7:Math.sin((t-.6)/.4*Math.PI)*.32*(1-t)/.4;
        die.position.set((i-(rolls.length-1)/2)*1.45+(i%2?1:-1)*remaining*.4,drop,0);
      });
