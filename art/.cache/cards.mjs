...
for (const id of Object.keys(PATCH26)) {
    if (DB[id])
        Object.assign(DB[id], PATCH26[id]);
}
...
// ============================================================
export const BALANCE_VERSION = "v26"; // v26: 함정 리워크 2차 — T8 부패 부여(decaytrap) · T9 바운스(undertow) · T6 코스트 데미지(boltcost) · GT5_1 전체 공격 봉쇄(gateLockAll) · NT_NULL4 마법 복제 강탈(spellSteal, cap4) · GT6_2 드로우 -2(omen)
...
export function frameFor(t) {
    if (t === "mon")
        return "/frames/red.png";
    if (t === "trap")
        return "/frames/green.png";
    if (t === "starter")
        return "/frames/cyan.png";
...
const SL = (id, ko, ja, en, price) => ({ id, url: `/frames/sleeves/${id}.webp`, ko, ja, en, price });
...
export const SLEEVE_LIST = Object.values(SLEEVES);
export function sleeveUrl(id) {
