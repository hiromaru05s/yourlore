...
function stdEn(c, s0) {
    ...
    if (c.act === "destroyMon")
        s = mark(s.replace(/enemy monsters?/g, "monster"), " (either side)");
    if ((c.act === "destroyTrap" && (c.val ?? 1) < 99) || c.onSummon === "breaktrap")
        s = mark(s.replace(/enemy set traps?/g, "set trap"), " (either side)");
    if (c.act === "destroyEnch")
        s = mark(s.replace(/enemy enchantments?/g, "enchantment"), " (either side)");
    if (c.id === "DIVINE")
...
export function standardizeCardTexts(pools) {
    for (const pool of pools) {
        for (const id of Object.keys(pool)) {
            ...
            if (c.text && c.text !== "—")
                c.text = ov?.ko ?? stdKo(c, c.text);
            if (c.textJa && c.textJa !== "—")
                c.textJa = ov?.ja ?? stdJa(c, c.textJa);
            if (c.textEn && c.textEn !== "—")
                c.textEn = c.textEn === ko0 ? c.text : ov?.en ?? stdEn(c, c.textEn);
        }
    }
}
